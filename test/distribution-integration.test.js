...
test("Cache enthält die sechs K3.6-Laufzeitdateien",()=>{assert.match(files.worker,/mathe-unterrichts-app-v44/);for(const file of runtimeFiles)assert.match(files.worker,new RegExp(file.replaceAll(".","\\.")));assert.doesNotMatch(files.worker,/render-distribution-states|distribution-(?:design|static\.test)/);});
