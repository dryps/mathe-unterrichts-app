...
test("Cache v32 enthält weiterhin die sechs K4.2-Laufzeitdateien",()=>{assert.match(files.worker,/mathe-unterrichts-app-v32/);assert.doesNotMatch(files.worker,/mathe-unterrichts-app-v25/);for(const file of runtimeFiles)assert.match(files.worker,new RegExp(file.replaceAll(".","\\.")));assert.doesNotMatch(files.worker,/render-both-sides-states|terme-beide-seiten-(?:design|static\.test)/);});
