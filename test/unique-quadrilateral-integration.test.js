...
test("Startseite integriert K5.4 genau einmal als vierte Kapitel-5-Karte",()=>{const chapter=files.home.match(/<section\s+id="vierecke"[\s\S]*?<\/section>/)?.[0];assert.ok(chapter);assert.equal((chapter.match(/class="module-card"/g)??[]).length,4);assert.equal((files.home.match(/href="\.\/eindeutige-vierecke\.html"/g)??[]).length,1);assert.match(chapter,/Warum legen manche Angaben ein Viereck eindeutig fest – und andere nicht\?/);assert.match(chapter,/<span class="module-subtitle">Vierecke konstruieren<\/span>/);assert.equal((files.home.match(/class="module-card"/g)??[]).length,31);});

test("Cache v36, Pages und Smoke enthalten exakt die sechs neuen Laufzeitdateien",()=>{assert.match(files.worker,/mathe-unterrichts-app-v36/);assert.doesNotMatch(files.worker,/mathe-unterrichts-app-v30/);for(const file of runtimeFiles){const pattern=new RegExp(file.replaceAll(".","\\."));assert.match(files.worker,pattern);assert.match(files.runtime,pattern);assert.match(files.smoke,pattern);}assert.doesNotMatch(files.worker,/render-unique-quadrilateral-states|unique-quadrilateral-(?:design|static\.test)/);});
