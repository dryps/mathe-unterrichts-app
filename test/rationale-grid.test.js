...
test("Kapitel 1 enthält genau vier Karten ohne leeres Rasterelement", () => {
...
test("Kapitel 1 besitzt außerhalb der kleinen Breite ausschließlich zwei gleich große Spalten", () => {
  assert.match(
    css,
    /\.chapter-rationale \.module-grid\s*\{\s*grid-template-columns: repeat\(2, minmax\(0, 1fr\)\)/,
  );
  assert.match(
    landscapeCss,
    /\.chapter-rationale \.module-grid\s*\{[\s\S]*grid-template-columns: repeat\(2, minmax\(0, 1fr\)\)/,
  );
  assert.match(mobileCss, /\.chapter-rationale \.module-grid\s*\{\s*grid-template-columns: 1fr/);
  assert.match(portraitCss, /grid-template-columns: repeat\(2, minmax\(0, 1fr\)\)/);
});

test("Kapitel 1 erzeugt im Querformat keine impliziten Spalten", () => {
  assert.match(landscapeCss, /\.chapter-rationale \.module-card\s*\{\s*grid-column: auto/);
