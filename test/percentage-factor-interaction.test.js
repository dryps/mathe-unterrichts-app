...
async function harness({reduced=true}={}){
  const ids=["pf-workspace","pf-chain","pf-rate","pf-hundredth-step","pf-reduced-step","pf-decimal-step","pf-hundredth","pf-reduced","pf-decimal","pf-product","pf-warning","pf-warning-title","pf-warning-text","pf-explore","pf-conclusion","pf-scenario","pf-scenario-output","pf-insight","pf-live","pf-next","pf-reset"];
...
test("Erkundung aktualisiert die ganze Kette und den zugänglichen Reglernamen",async()=>{
  ...
  next.dispatch("click");
  assert.equal(elements.get("#pf-hundredth-step").hidden,false);
  assert.equal(elements.get("#pf-reduced-step").hidden,true);
  assert.equal(elements.get("#pf-decimal-step").hidden,true);
  for(let step=1;step<5;step+=1) next.dispatch("click");
  ...
  assert.equal(elements.get("#pf-warning-title").textContent,"Warum nicht 75 · 40?");
  assert.equal(elements.get("#pf-warning-text").textContent,"75 % bedeutet 75 / 100 – der Faktor ist 0,75.");
...
test("Mehrfachtipp und Reset neutralisieren alte Rückrufe",async()=>{
  ...
  assert.equal(elements.get("#pf-hundredth-step").hidden,true);
});
